...
var Save = function() {
	...
	$.post('/goods/edit', data, function(res) {
		toastr.options = {
			"closeButton": false,
			"debug": false,
...
var Delete = function() {
	...
	if (id != 'new') {
		if (confirm("Вы действительно хотите удалить товар?") == true) {
			$.post('/goods/delete', {
				id: id
			}, function(res) {
				window.location.href = '/goods';
...
var InitFileUploadForn = function() {
	...
	$.ajax({
		type: 'POST',
		url: '/goods/photos/get?id=' + $('#product-id').val(),
		dataType: 'json',
